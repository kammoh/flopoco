...
Each signal now will have a (cycle, criticalPath) associated to it (with a default of 0 for signals that do not add to the critical path).
The semantics is: 
...
Instance: 
TODO add instanceName to inPortMap et outPortMap
...
Operator constructor builds the signal graph.
outputVHDL does all the synchronisation, cycle management, etc, and calls outputVHDL of instances.
