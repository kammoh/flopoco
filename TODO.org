...
* Version 5
** Target -> OperatorContext
Here is the work plan:
1/ sed global replace target-> with getTarget()->
findReplace("src", "target->", "getTarget()->", "*.cpp")
findReplace("src", "target ->", "getTarget()->", "*.cpp")

  (should be mostly harmless)
2/ create OperatorContext class including a target, a ParentOp, and a frequency
  (should be mostly harmless)
3/ sed global replace Target* target  with OperatorContext* context
+ add the indirection in Operator::getTarget()
4/ get rid of the mailbox and get back to work.
