...
namespace flopoco{
  ...
  PIFAdd::PIFAdd(Target* target, Operator* parentOp, int wE, int wF): Operator(parentOp, target), wE_(wE), wF_(wF) {
    ...
    vhdl << declare(.0, "is_larger_exp", 1, false) << "<= '1' when X_exponent > Y_exponent else '0';" << endl;
    
    vhdl << "with is_larger_exp select " << declare(.0, "larger_exp", wE) << "<= " << endl <<
      tab << "X_exponent when '1'," << endl <<
      tab << "Y_exponent when '0'," << endl <<
      tab << "\"" << string(wE, '-') << "\" when others;" << endl;
    
    vhdl << "with is_larger_exp select " << declare(.0, "smaller_exp", wE) << "<= " << endl <<
      tab << "Y_exponent when '1'," << endl <<
      tab << "X_exponent when '0'," << endl <<
      tab << "\"" << string(wE, '-') << "\" when others;" << endl;
    
    vhdl << "with is_larger_exp select " << declare(.0, "larger_mantissa", wF+2) << "<= " << endl <<
      tab << "X_s & X_fraction when '1'," << endl <<
      tab << "Y_s & Y_fraction when '0'," << endl <<
      tab << "\"" << string(wF+2, '-') << "\" when others;" << endl;
    
    vhdl << "with is_larger_exp select " << declare(.0, "smaller_mantissa", wF+2) << "<= " << endl <<
      tab << "Y_s & Y_fraction when '1'," << endl <<
      tab << "X_s & X_fraction when '0'," << endl <<
      tab << "\"" << string(wF+2, '-') << "\" when others;" << endl;

    vhdl << "with is_larger_exp select " << declare(.0, "larger_sign", 1, false) << "<= " << endl <<
      tab << "X_s when '1'," << endl <<
      tab << "Y_s when '0'," << endl <<
      tab << "'-' when others;" << endl;
    
    vhdl << "with is_larger_exp select " << declare(.0, "pad", 1, false) << "<= " << endl <<
      tab << "Y_s when '1'," << endl <<
      tab << "X_s when '0'," << endl <<
      tab << "'-' when others;" << endl;

    vhdl << declare(.0, "offset", wE+1) << " <= ('0' & larger_exp) - ('0' & smaller_exp);" << endl;

    vhdl << declare(.0, "sup_offset", wE+1-maxshiftsize) << " <= offset" << range(wE, maxshiftsize) << ";" << endl;

    vhdl << declare(.0, "saturate", 1, false) << " <= '0' when sup_offset = \"" << string(wE+1-maxshiftsize, '0') << "\" else '1';"<<endl;

    vhdl << "with saturate select " << declare(.0, "inf_offset", maxshiftsize) << "<=" << endl <<
    ...
    vhdl << "with larger_sign select " << declare(.0, "padded_larger_mantissa", wF+4) << "<= " << endl << //ça fait trop paddé y'a un pb 
      tab << "\"0\" & larger_mantissa & '0' when '0'," << endl <<
      tab << "\"1\" & larger_mantissa & '0' when '1'," << endl <<
      tab << "\"" << string(wF+4, '-') << "\" when others;" << endl;
    
    vhdl << declare(.0, "add_mantissa", wF + 6) <<  "<= ((padded_larger_mantissa) + (pad & shifted_frac_trunc)) & shifted_frac"<< of(0) << " & sticky;" << endl;
    ...
    vhdl << declare(.0, "exponent", wE) << " <= larger_exp + 2 - lzCount;" << endl;
    
    vhdl << declare(.0, "round", 1, false) << "<= significand"<< of(4) << ";" << endl;
    
    vhdl << declare(.0, "not_sticky", 1, false) << "<= '1' when significand" << range(3, 0) << " = \"0000\" else '0';" << endl;
    vhdl << declare(.0, "pre_is_zero", 1, false) << "<= (not round) or (not_sticky and round);" << endl;
    vhdl << declare (.0, "fraction", wF+1) << "<= significand" << range(wF+5, 5) << ";" << endl;
    vhdl << declare(.0, "fraction_is_zero", 1, false) << "<= '1' when fraction = \"" <<  string(wF+1, '0') << "\" else '0';" << endl;
    vhdl << declare(.0, "is_zero", 1, false) << "<= not count_type and fraction_is_zero and pre_is_zero;" << endl;
    
    vhdl << "with is_zero select " << declare(.0, "s", 1, false) << "<= " << endl <<
      tab << "'0' when '1'," << endl <<
      tab << "not fraction" << of(wF) << " when '0'," << endl <<
      tab << "'-' when others;" << endl;
    
    vhdl << "with is_zero select " << declare(.0, "final_exponent", wE) << "<= " << endl <<
      tab << "\"" << string(wE, '0') << "\" when '1'," << endl <<
      tab << "exponent when '0'," << endl <<
      tab << "\"" << string(wE, '0') << "\" when others;" << endl;
    
    vhdl << declare(.0, "is_NAR", 1, false) << " <= X" << of(width-1) << " or Y" << of(width-1) << ";" << endl;
