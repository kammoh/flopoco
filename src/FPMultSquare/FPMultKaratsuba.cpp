...
//				vhdl << tab <<"                      sigProd" << range(wFX_+wFY_-1, wFX_+wFY_ - wFR_) << ";"<<endl;
	...
	void FPMultKaratsuba::registerFactory(){
		UserInterface::add("FPMultKaratsuba", // name
											 "A floating-point multiplier using the Karatsuba algorithm.",
											 "operator; floating point; floating-point adders", // categories
											 "wE(int): exponent size in bits; \
                        wF(int): output's mantissa size in bits; \
                        norm(bool): do you want to normalize the result;",
											 "",
											 FPMultKaratsuba::parseArguments
											 ) ;
