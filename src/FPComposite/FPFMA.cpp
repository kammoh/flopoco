...
#define DEBUGVHDL 0
	...
	FPFMA::FPFMA(OperatorPtr parentOp, Target* target, int wE_, int wF_, bool ieee_compliant_) :
		Operator(parentOp, target), wE(wE_), wF(wF_), ieee_compliant(ieee_compliant_) {
	...
	void FPFMA::emulate(TestCase * tc)
	{
		/* Get I/O values */
		...
		if(ieee_compliant) {
			...
			throw std::string("FPFMA::emulate: Not yet implemented for FloPoCo numbers");
		}
	}
	...
	void FPFMA::buildStandardTestCases(TestCaseList* tcl){
		TestCase *tc;
	...
	void FPFMA::buildRandomTestCases(TestCaseList* tcl, int n){
		TestCase *tc;
	...
	OperatorPtr FPFMA::parseArguments(OperatorPtr parentOp, Target *target, vector<string> &args) {
		int wE, wF;
		bool ieee;
		UserInterface::parseStrictlyPositiveInt(args, "wE", &wE); 
		UserInterface::parseStrictlyPositiveInt(args, "wF", &wF);
		UserInterface::parseBoolean(args, "ieee", &ieee);
		return new FPFMA(parentOp, target, wE, wF, ieee);
	}

	void FPFMA::registerFactory(){
		UserInterface::add("FPFMA", // name
