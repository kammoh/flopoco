...
void usage(char *name, string opName = ""){
	...
	if ( full || opName == "IntAdderShortLatency" || opName == "IntAdder"){
		OP("IntAdderShortLatency","wIn optimizeType srl inputDelay");
		cerr << "Integer adder, multiple parameters, possibly pipelined\n";
		cerr << "optimizeType=<0,1,2,3> 0=LUT 1=REG 2=SLICE 3=LATENCY\n";
		cerr << "srl=<0,1> Allow SRLs\n";
		cerr << "inputDelay\n";
	...
	do {
		...
		else if(opname=="FixHalfSine")
		{
			if (i+2 > argc)
				...
				int lsb = atoi(argv[i++]);
				int N = checkStrictlyPositive(argv[i++], argv[0]);
				op = new FixHalfSine(target, lsb, N);
				addOperator(op);
			}
		}
		...
		else if(opname=="IntAdderShortLatency"){
			int nargs = 4;
			if (i+nargs > argc)
				usage(argv[0],opname);
			else {
				...
				op = new IntAdderShortLatency(target, wIn, delayMap, type, srl);
