...
namespace flopoco{
	...
	public:
		...
		*/
		 ...
		 * @param isSymmetric	the coe coefficients of the filter are symmetric, or not
		 * @param rescale
		 *						If rescale=false, the msb of the output is computed so as to avoid overflow.
		 *						If rescale=true, all the coefficients are rescaled by 1/sum(|coeffs|).
		 * This way the output is also in [-1,1], output size is equal to input size, and the output signal makes full use of the output range.
		*/
		FixFIR(Target* target, int lsbIn, int lsbOut, vector<string> coeff, bool isSymmetric=false, bool rescale=false, map<string, double> inputDelays = emptyDelayMap);
	...
	protected:
		...
		FixSOPC *fixSOPCSymm;				/**< the SOPC used for implementation, in the case of a symmetric filter */
	};
