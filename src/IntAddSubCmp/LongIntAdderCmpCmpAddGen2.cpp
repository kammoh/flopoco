...
namespace flopoco{
		...
		Operator(target), wIn_(wIn), inputDelays_(inputDelays)
	{
		...
		setNameWithFreqAndUID(join("LongIntAdderCmpCmpAddGen2_", wIn_));
...
//#define test512
	...
	OperatorPtr LongIntAdderCmpCmpAddGen2::parseArguments(Target *target, vector<string> &args) {
		int wIn;
		UserInterface::parseStrictlyPositiveInt(args, "wIn", &wIn);
		return new LongIntAdderCmpCmpAddGen2(target, wIn);
	}

	void LongIntAdderCmpCmpAddGen2::registerFactory(){
		UserInterface::add("LongIntAdderCmpCmpAddGen2", // name
											 "An long integer comparator for experimentation.",
											 "BasicInteger", // categories
											 "",
											 "wIn(int): input size in bits;",
											 "",
											 LongIntAdderCmpCmpAddGen2::parseArguments
											 ) ;
