...
//			addImplementationList.push_back(intAdderInstantiation);
		...
		//	this is not working anymore, due to the links of the signals to their parent operator
		//	so we need to create a deep copy instead
		//cloneOperator(addImplementationList[selectedVersion]);
		deepCloneOperator(addImplementationList[selectedVersion]);
		changeName(name.str());
